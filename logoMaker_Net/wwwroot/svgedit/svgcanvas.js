...
// This function resets the svgcontent element while keeping it in the DOM.
    var clearSvgContentElement = canvas.clearSvgContentElement = function () {
        ...
        var comment = svgdoc.createComment(" Created with Tagbit.co - https://www.Tagbit.co/ ");
...
// Implement the svgedit.history.HistoryEventHandler interface.
    ...
    var addCommandToHistory = this.addCommandToHistory = function (cmd) {
...
//			if (j == 0) selectedBBoxes[0] = svgedit.utilities.getBBox(elem);
        ...
        if (showGrips || selectedElements.length == 1) {
            if (selectedElements[0].tagName == "text") {
                selectorManager.requestSelector(selectedElements[0]).showGrips(false, true);
            } else {
                    ...
                    if (selectedElements[0] != null) {
                        ...
                        if (selectedElements[1] == null) {
                            ...
                            if (selected.tagName == "text") {
                                ...
                                selectorManager.requestSelector(selected).showGrips(false, true);
                            } else {
...
//							call("selected", [selected]);
                    ...
                    if (selected != undefined && selected.tagName == "text") {
                        canDisSelect = false;
                        dblClick(evt);
                    }
                    return;
...
//				}
            ...
            toEditMode: function (x, y) {
                curtext.oldText = curtext.textContent;
                allow_dbl = false;
                current_mode = "textedit";
                selectorManager.requestSelector(curtext).showGrips(false, true);
...
//				}
                ...
                /* modified by hari */
                if (pt != undefined) {
                    setCursorFromPoint(0, pt.y);
                }
                setEndSelectionFromPoint(10000, y, true);
            },
            toSelectMode: function (selectElem) {
                var cmd = new svgedit.history.ChangeElementCommand(curtext, {"#text": curtext.oldText}, {"#text": curtext.textContent});
                if (curtext.oldText != curtext.textContent) {
    ...
    this.embedImage = function (val, callback) {
        ...
        $(new Image()).load(function () { 
            ...
            try {
                ...
                encodableImages[val] = canvas.toDataURL();
            } catch (e) {
                encodableImages[val] = false;
            }
            last_good_img_url = val;
            if (callback) {
                callback(encodableImages[val], this);
            }
        }).attr('src', val);
    };
...
// * import should happen in top-left of current zoomed viewport
    this.importSvgString = function (xmlString, attribs) {
        if (!attribs){
            attribs = {
                pos: {}
            };
        ...
        try {
            ...
            use_el.id = attribs.id || getNextId();
            ...
            clearSelection();
            if (attribs.pos.x) {
                use_el.setAttribute("x", attribs.pos.x);
            }
            if (attribs.pos.y) {
                use_el.setAttribute("y", attribs.pos.y);
            }
            if (attribs.pos.height) {
                use_el.setAttribute("height", attribs.pos.height);
            }
            if (attribs.pos.width) {
                use_el.setAttribute("width", attribs.pos.width);
            }
            if (attribs.transform) {
                use_el.setAttribute("transform", attribs.transform);
            }
...
// val - Float with the new font size
    ...
    this.setDefaultFontSize = function (val) {
        cur_text.font_size = val;
    };
// Function: getText
